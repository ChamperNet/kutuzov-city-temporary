...
document.addEventListener('click', function(event) {
    ...
    const isClickInsideModal = clickedElement.closest('.modal-wrapper') !==
        null;
    const isClickOnOpenModalButton = clickedElement.closest(
        '._js-open-modal') !== null;
...
window.addEventListener('DOMContentLoaded', function() {
    [].forEach.call(document.querySelectorAll('.tel'), function(input) {
        ...
        function mask(event) {
            ...
            reg = new RegExp('^' + reg + '$');
            if (!reg.test(this.value) || this.value.length < 5 || keyCode >
                47 && keyCode < 58) {
...
document.querySelector('.modal-form').
    addEventListener('submit', async function(e) {
        ...
        try {
            ...
            alertMessage.textContent = 'Заявка успешно отправлена!';
